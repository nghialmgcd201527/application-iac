locals {
  ...
  web_domain      = local.environment_vars.locals.web_admin_domain
  web_bucket      = local.environment_vars.locals.web_admin_bucket
...
terraform {
  source = "${path_relative_from_include()}//modules/s3-staticweb-multitanent"
}
...
dependency "waf_web_identity" {
  config_path = find_in_parent_folders("waf_web_admin")
  mock_outputs = {
...
dependencies {
  paths = ["../../acm_datahouseasia", "../waf_web_admin"]
}
...
inputs = {
  ...
  waf_acl_id              = dependency.waf_web_identity.outputs.waf_arn
